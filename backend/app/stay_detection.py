    ...
    for i in range(0, len(clustered_gps_trajectory), 1):
        ...
        # 다른 cluster로 지정
            ...
            if cluster_cnt >= min_cluster_num:
                cluster_list = np.insert(cluster_list, len(cluster_list), [cluster_num, start_timestamp, end_timestamp ,start_latitude, start_longtitude, sum_latitude / cluster_cnt, sum_longtitude / cluster_cnt, cluster_cnt, rd.randint(1,20)], axis=0)
