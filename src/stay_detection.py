...
def stay_detection(gps_trajectory):
    ...
    cluster_cnt_list = np.array([[0,0,0,0]], dtype= np.float64)
    eps = 0.000009 * 5
    ...
    for i in range(0, len(clustered_gps_trajectory), 1):
        ...
        if i == 0:
            ...
            cluster_cnt += 1
            cluster_cnt_list = np.insert(cluster_cnt_list, len(cluster_cnt_list), [cluster_num, start_latitude, start_longtitude, cluster_cnt], axis= 0)
            cluster_cnt_list = np.delete(cluster_cnt_list, 0, axis= 0)
        else:
            ...
            # 다른 cluster로 지정
                ...
                if cluster_num == 1:
                    np.place(cluster_cnt_list, [cluster_num, start_latitude, start_longtitude, cluster_cnt], [cluster_num, start_latitude, start_longtitude, cluster_cnt])
                else:
                    cluster_cnt_list = np.insert(cluster_cnt_list, len(cluster_cnt_list), [cluster_num, start_latitude, start_longtitude, cluster_cnt], axis=0)
                cluster_cnt = 1
                cluster_num += 1
